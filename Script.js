...
function updateTask2(matricula, nombre, edad, sexo, promedio, key) {
    datos.ref('Mujer/' + key).update({
        matricula: matricula,
        nombre: nombre,
        edad: edad,
        sexo: sexo,
...
function onClickUpdate() {
    ...
    if (matricula.length == 0 || nombre.length == 0 || edad.length == 0 || sexo.length == 0 || promedio.length == 0) {
        ...
        if (sexo == "Hombre") {
            ...
            inHTML("loadTable", "");
            updateTask2(matricula, nombre, edad, sexo, promedio, key);
            inHTML("editData", "");
            alert("modify successfully");
            update.disabled = true;
        }
...
function removeTask2(key) {
    if (confirm("¿Deseas borras este alumno?")) {
        inHTML("loadTable2", "");
        datos.ref('Hombre/' + key).remove();
    }
}
...
function table2(matricula, nombre, edad, sexo, promedio, key) {
    return '<tr><td>' + matricula + '</td><td>' + nombre + '</td><td>' + edad + '</td> <td>' + sexo + '</td> <td>' + promedio + '</td>' +
        '<td><a href="#" onclick="viewDataUpdate(\'' + matricula + '\',\'' + nombre + '\', \'' + edad + '\', \'' + sexo + '\', \'' + promedio + '\', \'' + key + '\')">' +
        '<i class="fas fa-edit blue icon-lg"></i></a></td>' +
        '<td><a href="#" onclick="removeTask2(\'' + key + '\')">' +
        '<i class="fas fa-trash-alt red icon-lg"></i></a></td></tr>';
}
